...
export class Console extends PureComponent<ConsoleProps, ConsoleState> {
    ...
    connect = (host: string, port: number, password: string) => {
        ...
        this._webSocket.on('connect', () => this.setState({connected: true}))
        .on('auth', () => this.setState({auth: true, output: [...this.state.output, 'Authentication Successful']}))
        .on('response', (res: string) => this.setState({output: [...this.state.output, res]}))
        .on('disconnect', () => this.setState({connected: false, auth: false}))
        .on('error', (err: Error) => {this.setState({output: [...this.state.output, 'ERROR']}); throw err;});
    }
        ...
        return (
            <>
                <div className="shell-wrap">
                    ...
                    <ul className='shell-body'>{this.state.output.map((str, key) => (<li key={key}>{str}</li>))}</ul>
                    <form onSubmit={this.handleSubmit} className='shell-form'>
                        <input type="text" className="shell-command-input" onChange={this.handleChange} value={this.state.command} onKeyDown={this.keyDownHandler} />
